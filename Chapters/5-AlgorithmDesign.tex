...
The other chromosome is used to determine when to pay to get out of jail, and
...
\label{alg_buyHouse}
\begin{algorithmic}
\IF{player doees not have monopoly} 
  \STATE exit
\ENDIF

\IF{player does not have sufficient cash} 
  \STATE exit
\ENDIF
...
However, when it came time to play-test the evolved computer players against
...
slightly to be 1024. Based on previous studies of competitive fitness
...
(MSET). With two player games, SET pairs the players in a generation, competes
...
The number of players in a generation is set to a multiple of four so that MSET
is easier to implement. Population numbers that are not multiples of four would
require implementing the simulator to give some players byes\footnote{A bye is
when a participant in a tournament advances to the next level automatically
without playing a game at the current level.} for some matches, and keeping
track of that information. It is easier just to specify that the population size
is a multiple of four, so that no byes are necessary. So that is why Frayn's
population of 1000 was changed to 1024 for this research.
