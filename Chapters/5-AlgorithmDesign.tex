...
\begin{itemize}
  ...
  originally, can bid for the property in an action. The player must then decide
  whether to participate in the auction and how much to bid for the property.}
...
array (See Table \ref{table-chromo}). In this thesis, this chromosome is
...
Based on the game strategies listed earlier (Chapter \ref{m_gamestrategies})
...
The value at the element is a double which represents the probability that the
...
the R in RGA, or the S in SGA, the T in TGA has no meaning. It was chosen
because it follows R and S alphabetically.}.
...
\begin{itemize}
  ...
  \item Another 30\% of the new population is created by selecting individuals through
  tournament selection, mutating them, and then copying them into the new
  generation.
