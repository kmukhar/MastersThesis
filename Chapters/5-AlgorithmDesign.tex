...
machine for a prototype player. Not all transitions are shown here; for example,
a player can participate in an auction, trade properties, and develop
properties, from the inactive state}
\label{figure-statemachine}
\end{figure}

The decisions that a player makes are whether to buy a property or not (the
arrow numbered 1 in Figure~\ref{figure-statemachine}), whether to pay bail to
exit jail or not (the arrow numbered 2 in Figure~\ref{figure-statemachine}),
whether to buy houses for a property or not (the arrow numbered 3 in
