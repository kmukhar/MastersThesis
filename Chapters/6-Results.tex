...
In each trial, 50 games are played, so the expected score for any player is \(50
...
These results are also reasonable in terms of the game. Obviously there is a
...
monopolies a player owns. Two players, one with 3 Red properties and the other
with 3 Light Blue properties, have the same number of properties and the same
number of monopolies, but most experts would say the player with the Red
...
into the solution space~\cite{Panait02acomparative}. The average number of games
played per generation for Tournament was only 2. That, and the high degree of
randomness in each game, created a solution space with a large amount of noise,
which led to the poor performance of Tournament fitness evaluator.

Finally, although some of the results imply that NM and NP are similar in
performance, while other results imply that NP is better than NM, this is most
likely due to the confounding effect of having four players per game, rather
than a two player competition. The presence of stronger or weaker players in the
set of four players affects the results of the t-test between any two players.
...
computer players, the results are generally disappointing, but they do point to
...
the the kinds of trades proposed by human experts when gains and losses were
balanced \(\omega \approx 0.5\)), and when the threshold was relatively low
(approximately 100). They also claimed that when competed against each other,
their trading agents had the most wins with low threshold and balanced
\(\omega\).
...
preliminary testing showed that computer players proposed an excessive number of
trades with low threshold and balanced \(\omega\). The values in
Table~\ref{tab:val1_params} were chosen to provide a representative sampling of
combinations of trading parameters.
...
propose or accept trades when the gain was significant.
...
Out of 70 games, the human players won 53 times (\(\approx24\%\)). The best of
the computer players only managed to win 8 times.
...
games. The results are shown in Table~\ref{tab:human_results1b}. The human
players were again much better than the computer players, although the computer
players may have improved. The computer players won (\(\approx30\%\) of the
games this time. Player 667 improved from an 11\% win rate to a 16\% win rate;
Player 423 improved from 3\% to 14\%. Player 101, however, dropped from 10\% to
0\%, hurting the overall computer rate.
...
of the gene values evolved to that high of a value.
...
Section~\ref{figure-genome999} for example), that probability is not quite the
...
There was a total of 109 games played between human players and computer
players.
...
A post-competition examination of the results leads to the conclusion that this
poor performance in relation to the other players is probably because the
Generation 999 players are more likely to buy properties. Since they are more
likely to buy properties, they are more likely to have less available cash and
also more likely to trade their properties to the human players. When the human
players get a monopoly, it is the Generation 999 players that are more likely to
go bankrupt first, since they have less cash available to pay rents.

A second reason for the poor performance of the Generation 999 players is that,
just by chance, then tended to play more games against players from Generation
...
the computer players. In the first competition, the threshold and \omega were
fixed at relatively high values, and the computer players won \(\approx24\%\) of
the games. When the threshold was lowered and \omega made more balanced, they
won \(\approx30\%\) of the games. In this validation, when the values were set
to those recommended by Yasumura, they won \(\approx34\%\) of the games.
...
was again limited to the three best players from Generation 999.

However, it was thought that fixing the values of the trading parameters in
previous competitions, the players may have been stuck in poor locations in the
solution space. Allowing the parameters to fluctuate might be a way to improve
player performance. Even though Yasumura, et al, claim the best performance from
a balanced \omega and a low trading threshold, they did not validate these
parameters in a real world situation. The appropriateness of proposals was
evaluated by comparing what the computer proposed for trading to the proposal
made by a human expert in the same situation. In their paper, they indicate that
there were some differences between their computer agents and the human experts,
which the reasons for this difference are that, ``(1) the parameters in the
evaluation function are not tuned enough and (2) the expert is taking into
consideration elements other than the expected profit.''

So even though setting \omega to 0.5 and the threshold to 100 appears to have
improved the performance, these may not be the best parameters. The trading
algorithm was updated by randomly choosing \omega and the threshold for
...
\begin{table}[htbp]
  ...
  \caption{Add caption}
    ...
    \toprule
          & Values \\
    \midrule
    Row Labels & Average Finish Order \\
