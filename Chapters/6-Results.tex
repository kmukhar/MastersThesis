...
was 9, for the population size 1024.}. In this validation, various fitness evaluators
were compared to see which fitness evaluator evolved the best players.

Finally, different population sizes are compared. In this validation, we expect
that players from larger populations would tend to evolve into better players. The validation
shows that this expectation was correct.
...
It is very obvious with the FINISH\_ORDER fitness evaluator, for example.
Because the FINISH\_ORDER evaluator assigns 1, 2, and 3 points to the players in
a game, for a total of 6 points per game, the average fitness for any single
game is \(6/4\) or 1.5. So for \(n\) games the average fitness is \(n * 1.5\).
...
average score of 150. Figure~\ref{figure-RGA-G000-N100-FO-initial_fitness} shows
the fitness distribution for the first generation of an RGA population with the
FINISH\_ORDER fitness evaluator. The figure shows a binomial shaped distribution
which appears to be centered around the population average of 150. 
